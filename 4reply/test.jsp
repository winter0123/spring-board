...
<script>
	var temp_textarea = document.getElementById('summernote');
	function submitform(mode){
		if(mode == "write"){
			console.log(temp_textarea.value);
			setTimeout(function(){
				console.log(temp_textarea.value);
				myform1.action = "reply_write.jsp";	//updateDB로 myform 데이터 전송
				myform1.submit();
			}, 5000);
			// myform1.action = "reply_write.jsp";	//updateDB로 myform 데이터 전송
		}else if(mode == "delete"){
			myform1.action = "reply_delete.jsp";	//deleteDB로 myform 데이터 전송
		}
	// myform1.submit();
	}
$(document).ready(function() {
	temp_textarea = document.getElementById('summernote');
	console.log(temp_textarea);
...
<%
	...
	ResultSet rset = stmt.executeQuery("select * from reply where id = "+eachnum1+";"); //번호로 조회해서 데이터 뽑기
	rset.next();
...
<tr>
	...
	<td width=750 align=left><textarea id="summernote" name="content"><%= eachcontent %></textarea></td>
	<!-- 텍스트박스 형식으로 쓸때는 content로 씀 -->
</tr>
</table>
<%
	rset.close();
