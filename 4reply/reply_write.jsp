...
<%
   ...
   String todayup = formatter.format(new java.util.Date());
		...
		int max = 0;
		String sql = "SELECT MAX(id) FROM reply";					//기존 번호 부여가 최대값에 자동으로 +1되어 번호가 부여되기 때문에
	...
	if("INSERT".equals(key)) {
		stmt1.execute("insert into reply (title,date,content,rootid) values ('"
		+hantitle+"','"+hantoday+"','"+hancontent+"', "+(max+1)+");");

	}else{															//"INSERT"와 같은 key값일때는(신규제품 등록), reply_insert.jsp에서 받아온 값으로 구문 
		stmt1.execute("update reply set title = '"+hantitle+"', "+"content = '"+hancontent+"'"+"where id = "+ hannum +";");
