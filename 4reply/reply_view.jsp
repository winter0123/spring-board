...
<script>
	function submitform(mode){
		if(mode == "list"){
			myform.action = "reply_list.jsp";	//updateDB로 myform 데이터 전송
			myform.submit();
		}else if(mode == "update"){
			myform.action = "reply_update.jsp";	//deleteDB로 myform 데이터 전송
			myform.submit();
		}else if(mode == "delete"){
			myform.action = "reply_delete.jsp";	//deleteDB로 myform 데이터 전송
			myform.submit();
		}else if(mode == "reinsert"){
			myform.action = "reply_reinsert.jsp";	//deleteDB로 myform 데이터 전송
			myform.submit();
		}
	}
</script>
...
<%
	...
	int status=1;
	try{   
		PreparedStatement ps =conn.prepareStatement("select * from reply where id = "+ckey+";");
	...
	PreparedStatement ps1 = conn.prepareStatement("update reply set viewcnt = '"+Hits+"' where id = "+ckey+";");
	status = ps1.executeUpdate();


	//후보자의 각 연령별 합계값을 알기 위한 rest값
	ResultSet rset = stmt.executeQuery("select * from reply where id = "+ckey+";");
...
<table border=1 cellspacing=0 style="margin-bottom:5px">
	<tr>
