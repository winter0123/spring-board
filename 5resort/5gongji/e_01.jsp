...
<body>
<center>
<h1 class="text_box1" align=center>News</h1>
<table class="text_box" border=1 cellspacing=0 style="background-color: #CCCCCC;">
<tr>
...
%>
<form method="post" action="5gongji_view.jsp">		<!-- 입력값 gongji_view로 전송 -->
<tr class="text_box" style="background-color: #ffffff;">
	<td width=50 align=center><%= newid %></td>		<!-- 번호 -->
	<td width=400 align=left><a href='5gongji_view.jsp?key=<%= newid %>'><%= newtitle %></a></td>
...
<form method="post" action="5gongji_insert.jsp">	<!--등록된 후보 추가 = A_03_H.JSP-->
		<tr>
...
if (index%cntperpage==0){
   ...
   out.println("<a href=\"e_01.jsp?bgn=1&cnt=" + cntperpage + "\" class=\"no-uline\"> << </a>");  // 첫 페이지로 가기
   out.println("<a href=\"e_01.jsp?bgn=" + (index - cntperpage) + "&cnt=" + cntperpage + "\"  class=\"no-uline\"> < </a>");   // 바로 전 페이지로 가기
   ...
   for (int i = 1; i <= 10; i++) {
      ...
      if (i == currentpage) {  // 현재 페이지이면
         out.println("<a href=\"e_01.jsp?bgn=" + (cntperpage*10*x+bgnindex) + "&cnt=" + cntperpage + "\"  class=\"active\"> " + i + "</a>");
         // style에서 정의한 active 클래스로 색 표시
         
      } else {  // 다른 페이지는
         out.println("<a href=\"e_01.jsp?bgn=" + (cntperpage*10*x+bgnindex) + "&cnt=" + cntperpage + "\"  class=\"no-uline\"> " + i + " </a>");
         // 일반
      }
   ...
   out.println("<a href=\"e_01.jsp?bgn=" + (cntperpage * (pages - 1) + 1) + "&cnt=" + cntperpage + "\" class=\"no-uline\"> > </a>");

   out.println("<a href=\"e_01.jsp?bgn=" + (cntperpage * (pages - 1) + 1) + "&cnt=" + cntperpage + "\" class=\"no-uline\"> >> </a>");
   }
   else {
   out.println("<a href=\"e_01.jsp?bgn=" + (index + cntperpage) + "&cnt=" + cntperpage + "\" class=\"no-uline\"> > </a>");
