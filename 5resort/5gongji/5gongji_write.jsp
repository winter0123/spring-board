...
<%
   ...
   String todayup = formatter.format(new java.util.Date());
	...
	if("INSERT".equals(key)) {
		stmt.execute("insert into 5gongji (title,date,content) values ('"+hantitle+"','"+hantoday+"','"+hancontent+"');");
	}else{															//"INSERT"와 같은 key값일때는(신규제품 등록) // gongji_insert.jsp에서 받아온 값으로 구문 
		stmt.execute("update 5gongji set title = '"+hantitle+"', "+"content = '"+hancontent+"'"+"where id = "+ hannum +";");
