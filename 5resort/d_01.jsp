...
<% 
	...
	conn.close();	
%>
   <h1><p align=center>예약시스템</p></h1>
   <table align=center cellspacing=1 width=800 border=1>
   <tr>
      <td width=150><p align=center><b>예약날짜</b></p></td>
      <td width=150><p align=center><b>VIP Room</b></p></td>
      <td width=150><p align=center><b>Suite Room</b></p></td>
      <td width=150><p align=center><b>Standard Room</b></p></td>
   </tr>
<%
	for (int i=0; i<resv_arr.length;i++){
			out.println("<tr align=center><td>"+resv_arr[i][0]+" ("+resv_arr[i][1]+")</td>");
			if(resv_arr[i][2].equals("예약가능")){
			out.println("<td><a href='d_02.jsp?key="+resv_arr[i][0]+"&room=1'>"+resv_arr[i][2]+"</a></td>");
			}else{
			out.println("<td>"+resv_arr[i][2]+"</td>");
			}
			if(resv_arr[i][3].equals("예약가능")){
			out.println("<td><a href='d_02.jsp?key="+resv_arr[i][0]+"&room=2'>"+resv_arr[i][3]+"</a></td>");	
			}else{
			out.println("<td>"+resv_arr[i][3]+"</td>");	
			}
			if(resv_arr[i][4].equals("예약가능")){
			out.println("<td><a href='d_02.jsp?key="+resv_arr[i][0]+"&room=3'>"+resv_arr[i][4]+"</a></td></tr>");
			}else{
